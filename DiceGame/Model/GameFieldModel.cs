...
namespace DiceGame.Model
{
    public class GameFieldModel : IGameFieldModel
    {
        ...
        public IList<IPieceModel> Pieces { get; } = new List<IPieceModel>();
        public IPlayerModel[] Player { get; } = new IPlayerModel[8];





        public void ResetChangedPieces()
        {
        ...
        public GameFieldModel(IPlayerModel[] player)
        {
            for(int i = 0; i < player.Length && i < Player.Length; i++)
            {
                Player[i] = player[i];
            }
        ...
        private void InitFieldAndPieces()
        {
